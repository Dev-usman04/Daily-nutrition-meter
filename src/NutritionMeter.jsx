...
const NutritionMeter = () => {
  ...
  const removeAllItems = () => setNutritionItems([]);
  ...
  const totalProtein = () =>
    nutritionItems
      .reduce((total, item) => total + parseFloat(item.protein) * item.quantity, 0)
      .toFixed(2);

  const totalCarbs = () =>
    nutritionItems
      .reduce((total, item) => total + parseFloat(item.carbs) * item.quantity, 0)
      .toFixed(2);

  const totalFat = () =>
    nutritionItems
      .reduce((total, item) => total + parseFloat(item.fat) * item.quantity, 0)
      .toFixed(2);

  const inputErrorStyle = { borderColor: "red" };

  return (
    <div className="bg-green-900 min-h-screen flex items-center justify-center px-4">
      <div className="container mx-auto p-4 bg-white rounded-lg shadow-lg">
        ...
        <AnimatePresence>
          {showWarning && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="bg-red-500 text-white p-2 rounded-md text-center mb-4"
            >
        ...
        <div className="mb-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {["name", "calories", "protein","carbs", "fat"].map((key, i) => (
              <div key={i}>
                <input
                  ...
                  placeholder={
                    key === "name"
                      ? "Item Name"
                      : `${key.charAt(0).toUpperCase() + key.slice(1)}${key !== "calories" ? " (g)" : ""}`
                  }
                  className={`w-full py-2 px-3 border rounded-md focus:outline-none focus:ring focus:border-blue-300 ${
                    inputError && (!newItem[key] || newItem[key] < 0) ? "border-red-500" : ""
                  }`}
                  style={
                    inputError && (!newItem[key] || newItem[key] < 0)
                      ? inputErrorStyle
                      : {}
                  }
                  value={newItem[key]}
          ...
          <div className="mt-3 flex justify-between">
            {editItem ? (
              <button
                className="bg-blue-500 text-white p-3 rounded-md hover:bg-blue-600 mb-4 font-semibold focus:outline-none text-xs"
              ...
              <button
                className="bg-green-600 text-white p-3 rounded-md hover:bg-green-700 mb-4 font-semibold focus:outline-none text-xs"
            ...
            <button
              className="bg-red-600 text-white p-3 rounded-md font-semibold mb-4 hover:bg-red-700 focus:outline-none text-xs"
            ...
            {nutritionItems.map((item) => (
              ...
              >
                ...
                <ul className="mt-3">
                  ...
                  <li>Protein: {(item.protein * item.quantity).toFixed(2)}g</li>
                  <li>Carbs: {(item.carbs * item.quantity).toFixed(2)}g</li>
                  <li>Fat: {(item.fat * item.quantity).toFixed(2)}g</li>
                ...
                <div className="mt-3 flex justify-between">
                  <button
                    className="bg-blue-500 text-white pd-2 rounded-md hover:bg-blue-600 font-semibold focus:outline-none text-xs"
                    onClick={() => editItemFunction(item)}
                  >
                    <FontAwesomeIcon icon={faEdit} /> Edit
                  </button>
                  <button
                    className="bg-red-500 text-white pd-2 rounded-md hover:bg-red-600 font-semibold focus:outline-none text-xs"
